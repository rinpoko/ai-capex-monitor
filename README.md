...
Macで起動したサーバーに、同じWi-Fi内のiPhoneから `http://<MacのIPアドレス>:8765` でアクセスできます（Macがスリープ中や同じWi-Fiにいないと使えません）。

## GitHub Pages版（Mac不要・どこからでもアクセス可能）

Macを起動していなくても使えるように、`docs/`配下は完全に独立した静的サイト版になっています。

- **公開URL**: https://rinpoko.github.io/ai-capex-monitor/
- データは `.github/workflows/update-data.yml` によって**GitHub Actionsが6時間ごとに自動取得**し、`docs/data/snapshot.json` を更新・コミットします（手動実行も可: `gh workflow run update-data.yml`）
- `docs/app.js` はローカル版（`app/static/app.js`）から分岐した軽量版で、バックエンドAPIを叩かず静的JSONを読むだけの構成です。ロジックや見た目を変える場合は**両方**を更新してください
- iPhoneでは同じURLをSafariで開き、「ホーム画面に追加」すればMac版と同様にアプリ化できます（Wi-Fi不要、外出先でも閲覧可）
- Reddit（SNS話題度の一部）はGitHub Actionsのクラウド環境からは403でブロックされる可能性が高く、StockTwits分のみ反映されることが多いです
